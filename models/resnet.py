...
class Shortcut(nn.Module):
    def __init__(self, in_planes, planes, expansion=1, kernel_size=1, stride=1, bias=False, mode='train'):
        ...
        self.grad1 = nn.Conv2d(in_planes, expansion*planes, kernel_size=kernel_size, stride=stride, bias=False)
        self.grad1.weight.data = torch.ones(self.grad1.weight.size())
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, mode='train'):
        ...
        self.mask1.weight.data = torch.ones(self.mask1.weight.size())
        self.grad1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.grad1.weight.data = torch.ones(self.grad1.weight.size())
        ...
        self.grad2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.grad2.weight.data = torch.ones(self.grad2.weight.size())
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, mode='train'):
        ...
        self.mask1.weight.data = torch.ones(self.mask1.weight.size())
        #self.grad1  = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        #self.grad1.weight.data = torch.ones(self.grad1.weight.size())
